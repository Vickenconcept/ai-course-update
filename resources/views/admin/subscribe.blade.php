    ...
    <div class="h w-[70%] mx-auto mt-20 space-y-6 p-10 shadow-md bg-slate-100">
        ...
        <div>
            <p class="text-right">Amount: <span
                    class="font-bold bg-green-800 text-white px-3 py-1 rounded-md">${{ optional($admin)->subscriptiion_amount ?? NAN }}</span>
            </p>
        </div>
        <div class="grid grid-cols-2 gap-10">
            @if (optional($admin)->use_paypal)
                <div id="paypal-button-container"></div>
            @endif
            @if (optional($admin)->use_stripe)
                <div>
                    <form action="{{ route('process-payment.update') }}" method="post" id="payment-form">
                        @csrf
                        <input type="text" name="amount" id="amount" class="hidden"
                            value="{{ optional($admin)->subscriptiion_amount }}">
                        ...
                        <div class="mt-4">
                            <button type="submit" @if (optional($admin)->super_admin_strip_secret == '' && optional($admin)->super_admin_strip_key =='')
                                disabled
                            @endif
