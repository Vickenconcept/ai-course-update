    ...
    <div class="bg-gray-50 px-4 py-10" x-data="{ childIsOpen: false, openShare: false, openCreate: false }">
        ...
        <section class="mt-20 w-full md:w-[70%] mx-auto">
            ...
            <div class="flex  mb-3">
                ...
                <div>
                    <form action="{{ route('courses.courseType', ['course' => $course->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <select class="rounded-md" name="course_type" id="course_type" onchange="this.form.submit()">
                            <option value="text" @if ($course->course_type == 'text') selected @endif>Text</option>
                            <option value="video" @if ($course->course_type == 'video') selected @endif>Video</option>
                        </select>
                    </form>
                </div>
            ...
            <div id="formContent">
                ...
                @else
                    @foreach ($course->lessons as $lesson)
                        <livewire:course-video :lesson="$lesson" :key="$lesson->id" x-data="{ open: childIsOpen }" />
                    @endforeach
                @endif
            </div>
